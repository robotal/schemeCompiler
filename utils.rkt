...
(define (eval-llvm llvm-str [run-bin #t] [output "bin"])
  ...
  (system (string-append clang++-path " combined.ll " " -o " output))
  (if run-bin
      (let ()
        (match-define `(,out-port ,in-port ,id ,err-port ,callback) (process (string-append "./" output)))
